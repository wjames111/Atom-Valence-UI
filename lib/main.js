...
const colorsAdded = [
 ...
 themeName + 'colorCaret',
 themeName + 'customBorder',
 themeName + 'colorBorder',
]
...
function changeColor() {
  ...
  let currentCaret = atom.config.get(themeName + 'customCaret')? '#d56f3e' : atom.config.get(themeName + 'colorCaret');
  let currentBadge = atom.config.get(themeName + 'customBadgeBorder')? 'darkcyan' : atom.config.get(themeName + 'badgeColor');
  let currentBorder = atom.config.get(themeName + 'customBorder')? '#212630' : atom.config.get(themeName + 'colorBorder');
