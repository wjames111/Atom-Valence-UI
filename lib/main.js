...
const themeName = 'atom-valence-ui.colors.'
...
function changeColor() {
  ...
  let currentAccent = atom.config.get(themeName + 'customAccent')? 'rgba(85, 239, 211, 1)' : atom.config.get(themeName + 'accentColor');
  ...
  fs.writeFile(`${__dirname}/../styles/colorSetting.less`, content.join("\r\n"), 'utf8', (error) => {         // write colorSetting.less file based on changed custom values
    if (error) {
      ...
      const amuPackage = atom.packages.getLoadedPackage('atom-valence-ui');
